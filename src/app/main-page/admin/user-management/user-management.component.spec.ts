...
describe('UserManagementComponent', () => {
  ...
  it('at start no group is selected', () => {
    const groupSelect =  el.query(By.css('#selectGroup'));

    const currentGroup = new Group('Selecciona grupo');
  ...
  it('at start no users in select user list', () => {
    const userSelect = el.query(By.css('#selectUser'));

    const currentUser = new User('', '', 'Selecciona usuario', '', false, [], []);
  ...
  it('at start add user button is disabled', () => {
    const addUserButton = el.query(By.css('#addUserButton'));
  ...
  it('add group button is enabled', () => {
    const addGroupButton = el.query(By.css('#addGroup'));
  ...
  it('at start update group button is disabled', () => {
    const updateGroupButton = el.query(By.css('#updateGroup'));
  ...
  it('at start delte group button is disabled', () => {
    const deleteGroupButton = el.query(By.css('#deleteGroup'));
  ...
  it('when group is selected, update group and delete group buttons are enabled',   () => {
    // Get group test data
    const group = testService.getGroupByName('Group1');

    const spyOnGroupSelect = spyOn(component, 'onGroupSelect').and.callThrough();
  ...
  it('when group is selected, users group should apear in list',   () => {
    ...
    const users = testService.getUsersByGroupId(group._id);
    ...
    const usersList = el.query(By.css('#usersList'));
  ...
  it('delete user should delete user from list', () => {
    const spyOnClickDelete = spyOn(component, 'onClickDelete').and.callThrough();

    const group = testService.getGroupByName('Group1');
    const users = testService.getUsersByGroupId(group._id);
    const deleteUser =  users[0];
  ...
  it('select user to add, and change group should unselect user to add and disable add button', () => {
    const spyGetUsersGroup = spyOn(userStorageService, 'getUsersGroup');
    const spyGetUsersWithoutGroup = spyOn(userStorageService, 'getUsersWithoutGroup');
  ...
  it('delete group should delete group from list', () => {
    const deleteGroupSpy = spyOn(groupStorageService, 'deleteGroup').and.callThrough();

    // Create mock group
    const groupMock = testService.getGroupByName('Group1');
